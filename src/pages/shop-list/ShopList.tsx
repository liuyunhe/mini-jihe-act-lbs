...
import { hideLoading, openLocation, showLoading, showToast, stopPullDownRefresh, useDidHide, usePullDownRefresh } from '@tarojs/taro'
  ...
  const getShop = async (isRefresh = false) => {
    ...
    // try {
      location((async res => {
        ...
        // }
        let { data: { data, msg, code } } = await getSalersNearbyList(params)
        if (code == 200) {
...
export default function ShopList () {
  ...
  <View className='shop-list-wrapper'>
    {
      (!shop || !shop.length) && !first && <View className='list-no-data'>
    ...
    {
      shop.map((item, index) => (
        <View className='shop-list-item' key={index} onClick={onItemClick.bind(null, item)}>
          ...
          <View className='shop-location'>
            ...
            <View className='location-distance'>{transDistance(item.distance)}</View>
          </View>
        </View>
      ))
    }
